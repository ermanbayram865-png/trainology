import { calculateBodyMassIndex, calculateCalorieRequirement, calculateHealthyWeightRange, calculateMacroDistribution, calculateProteinRequirement, calculateWaterRequirement, type ActivityLevel, type Goal, type ProteinActivityLevel, type ProteinGoal } from "@/lib/calculators";
...
const proteinActivity: Record<AnalysisActivityLevel, ProteinActivityLevel> = { low: "low", moderate: "moderate", high: "active" };
const proteinGoal: Record<Goal, ProteinGoal> = { lose: "fatLoss", maintain: "generalHealth", gain: "muscleGain" };
...
export function calculateHealthSnapshot(input: AnalysisInput): AnalysisResult {
  ...
  const protein = calculateProteinRequirement(input.weight, proteinGoal[input.goal], proteinActivity[input.activityLevel]);
  const macro = calculateMacroDistribution({ calories: calorie.recommendedCalories, goal: input.goal, proteinPreference: "standard", activityLevel: input.activityLevel });
  const water = calculateWaterRequirement(input.weight, input.activityLevel);
  ...
  return { body: { bmi, bmiCategory: bmiCategory(bmi), healthyWeightRange: calculateHealthyWeightRange(input.height) }, nutrition: { maintenanceCalories: calorie.tdee, targetCalories: calorie.recommendedCalories, targetRange, dailyProtein: protein.dailyProtein, proteinPerKg: protein.proteinPerKg, macro, water }, activity: activitySummary(input.weeklyTrainingDays, input.dailySteps), target: target(input.goal) };
}
